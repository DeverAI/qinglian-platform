...
## 2026-08-08 SID 握手安全漏洞修复与健壮性升级轮

- [完成] 修复 `app/routers/auth.py` `handshake_heartbeat`：原心跳端点仅更新活跃时间，未检测 sid 是否已被轮换，导致攻击者可对已吊销 sid 持续发送心跳维持会话。改为调用原子函数 `update_handshake_checked(sid, user_id)`，sid 已轮换返回 4034、sid 不属于当前用户返回 4033。
- [完成] 修复 `tests/test_security_hardening.py` `test_image_compression_and_original_point_gate`：测试未携带 X-SID 头且未发送握手心跳，被 `require_handshake` 守卫拦截返回 4033。补充 `X-SID` 头与一次握手心跳调用。
- [完成] 调用故障检测子 AGENT 对 SID 握手安全系统做全面健壮性审查，输出结构化报告，发现 1 项 Critical、5 项 Medium、6 项 Low、4 项 Suggestion。
- [完成] 修复 C1（配置项 DoS）：`app/database.py` 将 `threshold_handshake_timeout`/`threshold_handshake_fail_limit` 纳入 `DEFAULT_THRESHOLDS`（此前只能直接改 DB，绕过 API 校验）；新增 `THRESHOLD_MIN_VALUES` 字典定义硬下限；`app/routers/root.py` `update_thresholds` 增加硬下限校验；`app/handshake.py` `_get_timeout`/`_get_fail_limit` 强制 `max(val, hard_min)` 兜底，防止 root 误配 0/负值导致全站 DoS 或首次失败即轮换。
- [完成] 修复 M1（心跳端点不校验 sid 归属）：`app/handshake.py` 新增 `update_handshake_checked(sid, user_id)` 原子校验 sid 归属+轮换状态+更新活跃时间，避免 TOCTOU；`auth.py` `handshake_heartbeat` 改用该函数，sid 不属于当前用户返回 4033，防跨用户保活会话。
- [完成] 修复 M2（登出不校验 sid 归属）：`app/handshake.py` 新增 `clear_handshake_for_user(sid, user_id)` 仅在 sid 属于当前用户时清除；`auth.py` `logout` 改用该函数，防攻击者用自身令牌+受害者 sid 清除受害者会话（DoS）。
- [完成] 修复 M4（多 sid 并存未及时失效）：`app/handshake.py` 新增 `invalidate_user_sids(user_id)` 静默失效该用户所有旧 sid（不写系统通知，避免登录轰炸）；`auth.py` `_issue_session` 签发新 sid 前调用该函数，消除旧 sid 在 `_maybe_cleanup` 回收前（约 180s）仍可被 `require_handshake` 接受的复用窗口。
- [完成] 修复 M5（失败计数器内存放大）：`app/handshake.py` `_record_user_fail` 增加单用户失败记录上限 `_FAIL_LIST_MAX=200`，超过则保留最近记录；同时在失败路径调用 `_maybe_cleanup` 触发周期性清理。
- [完成] 修复 L1（状态端点不校验 sid 归属）：`app/handshake.py` 新增 `get_handshake_status_for_user(sid, user_id)`，sid 不属于当前用户时返回空状态（防信息泄露）；`auth.py` `handshake_status` 改用该函数。
- [完成] 修复 L2（TOCTOU）：`update_handshake_checked` 将"检查轮换+检查归属+更新活跃时间"合并为单一临界区操作。
- [完成] 修复 L4（登出不清理失败计数）：`clear_handshake_for_user` 在清除 sid 时同时 `_user_fails.pop(user_id, None)`。
- [完成] 修复 L5（前端后台标签心跳恢复迟缓）：`static/common.js` `HandshakeManager.init` 新增 `visibilitychange`（切回前台立即补发心跳）与 `window.focus`（窗口获焦补发心跳）监听，避免浏览器后台节流导致 sid 超时后用户切回需等待。
- [完成] 文档归档：`Design.md` 新增"SID 握手安全漏洞修复与自主升级"决策记录；`Techniques.md` 新增第 54 项技术实现；`FreqErr.md` 新增 SID 轮换检测与配置项 DoS 两类常见错误；`dev_log/20260808_SID握手安全漏洞修复与健壮性升级.md` 记录完整修复过程；`todo.md` 清空。
- [完成] 集成测试回归：`python -m pytest tests/ --tb=short` 61 个测试全部通过，Err.log 为空。
- [完成] 备份修改文件至 `backups/20260808_sid_security_hardening/`。

## 2026-08-08 风控监控后台轮

- [完成] 后端 `app/risk_control.py` 核心模块：实现风控全栈能力——
  - `log_action`：审计日志三处冗余备份（DB `risk_audit_logs` + 文件 `storage/risk_audit/YYYYMM.log` + 高危操作额外写 `Err.log`），防删除。
  - `classify_risk`：基于方法/路径/action 评估风险等级 info/warn/danger/critical；DELETE→danger、批量操作(batch/bulk)→critical、角色变更/封禁/服务开关/阈值修改→critical、设置修改→danger。
  - `is_ip_suspicious`：检测不常见 IP（不在白名单 + 近 7 天未出现过 + 非本地 IP + 与用户历史常用 IP 不同）。
  - `issue_2fa_challenge` / `verify_2fa`：2FA 挑战签发与校验，验证码 PBKDF2 哈希存储不存明文，配置邮箱则发邮件不返回明文（防中间人），3 次失败自动封禁 IP + 封禁用户。
  - `is_2fa_verified`：检查令牌是否已通过验证且在 5 分钟复用窗口内，绑定 actor_id 防 A 的令牌授权 B 的操作。
  - `ban_ip` / `unban_ip` / `is_ip_banned`：IP 封禁管理（永久，需 root 手动解封）。
  - `add_whitelist` / `remove_whitelist` / `is_whitelisted`：IP 白名单管理（信任 IP 跳过异常检测，但 critical 操作仍需 2FA）。
- [完成] 后端 `app/routers/risk_control.py` 路由：root 专属 API——仪表盘统计 `/stats`、审计日志查询 `/logs`（支持按风险等级/IP/操作筛选+分页）、IP 封禁列表 `/ip-bans`、手动封禁 `/ip-bans/manual`、解封 `/ip-bans/{ip}/lift`、白名单 CRUD `/whitelist`、2FA 触发 `/2fa/trigger` 与校验 `/2fa/verify`。
- [完成] 后端 `app/main.py` 中间件 `risk_control_boundary`：拦截后台路径（/api/admin、/api/root、/api/risk）所有访问——封禁 IP 直接阻断(4036)、每次操作记录到审计日志、critical 级操作或异常 IP 自动触发 2FA(4035)、异常 IP 且未配置邮件直接拦截(4037)、2FA 接口自身豁免 2FA 校验。
- [完成] 后端 `app/database.py`：新增风控四张表 `risk_audit_logs`/`risk_2fa_challenges`/`risk_ip_bans`/`risk_ip_whitelist` 及索引。
- [完成] 后端 `app/config.py`：新增 `RISK_AUDIT_DIR`/`RISK_2FA_EMAIL`/`RISK_2FA_TTL_MINUTES`/`RISK_2FA_MAX_FAILS` 配置。
- [完成] 前端 `static/common.js`：新增全局 `TwoFaManager`——4035 自动弹出 2FA 验证码模态框，服务器本地访问时自动填入验证码，邮箱模式用户手动输入，验证通过后存储令牌(sessionStorage, 5min 复用)并自动重试原请求；4036(封禁)/4037(异常拦截) 显示拦截提示；`authHeaders()` 自动附加 `X-2FA-Token`；`clearAuth()` 清理 2FA 令牌。
- [完成] 前端 `static/common.css`：新增 `.two-fa-code-display`/`.two-fa-error`/`.modal-btn.secondary` 样式。
- [完成] 前端 `static/risk_control.html`：Root 专属风控监控后台页面——仪表盘（7 项统计卡片 + 机制说明）、审计日志（风险等级/IP/操作筛选 + 分页表格）、IP 封禁（手动封禁 + 列表 + 解封）、IP 白名单（增删 + 列表），所有 critical 操作自动触发全局 2FA 模态。
- [完成] 前端 `static/admin.html`：Root tab 顶部新增「风控监控」卡片与 `/risk_control.html` 入口链接。
- [完成] 测试 `tests/conftest.py`：新增 `risk_2fa` 夹具——对指定用户签发并校验 2FA 挑战，返回可复用的 `X-2FA-Token`，供 critical 级操作测试使用；设置 `QLKIMI_ROOT_EMAIL=""` 使测试环境 2FA 验证码本地返回。
- [完成] 测试 `tests/test_api.py`：更新 `headers()` 支持 `two_fa` 参数；修复 `test_user_role_and_ban`/`test_root_service_toggle`/`test_root_ban_device`/`test_root_ban_admin` 添加 2FA 验证步骤；`test_static_pages` 新增 `/risk_control.html`。
- [完成] 测试 `tests/test_security_hardening.py`：修复 `test_root_can_change_ip_ban_minutes` 添加 2FA 验证步骤（IP 手动封禁与阈值修改均为 critical 级操作）。
- [完成] 集成测试回归：`python -m pytest tests/test_api.py tests/test_security_hardening.py -v --tb=short` 61 个测试全部通过。
